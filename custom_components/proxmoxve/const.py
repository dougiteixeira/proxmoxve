...
PROXMOX_NODE_SENSOR: Final[tuple[ProxmoxSensorDescription, ...]] = (
    ProxmoxSensorDescription(
        key="uptime",
        icon="mdi:database-clock-outline",
        unit_metric=ATTR_TIME,
        unit_imperial=ATTR_TIME,
        conversion=lambda x: ( dt_util.utcnow()-timedelta(seconds=x) if x>0 else None ),
...
PROXMOX_CALCULATED_SENSOR_TYPES: Final[tuple[ProxmoxSensorDescription, ...]] = (
    ...
    ProxmoxSensorDescription(
        ...
        native_unit_of_measurement=PERCENTAGE,
        conversion=lambda x: round(x * 100, 1),
        calculation=lambda x: x["mem"] / x["maxmem"],
        state_class=SensorStateClass.MEASUREMENT,
    ),
)
