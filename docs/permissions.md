...
|Read the QEMU guest agent: a VM's disk usage from inside the guest, and the guest file sensor|Read only|HomeAssistant.GuestAgent|VM.GuestAgent.Audit, plus VM.GuestAgent.FileRead for the file sensor (Proxmox VE 8: VM.Monitor for both)|
